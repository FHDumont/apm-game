...
if [ "x${APPDYNAMICS_ANALYTICS_AGENT_NAME}" != "x" ]; then
    replaceText 'ad.agent.name=analytics-agent1' "ad.agent.name=${APPDYNAMICS_ANALYTICS_AGENT_NAME}" $ANALYTICS_AGENT_PROPERTIES
fi
...
replaceText 'http.event.endpoint=http://localhost:9080' "http.event.endpoint=${APPDYNAMICS_ANALYTICS_EVENTS_SERVICE}" $ANALYTICS_AGENT_PROPERTIES

replaceText 'http.event.name=customer1' "http.event.name=${APPDYNAMICS_AGENT_ACCOUNT_NAME}" $ANALYTICS_AGENT_PROPERTIES

replaceText 'http.event.accountName=analytics-customer1' "http.event.accountName=${APPDYNAMICS_ANALYTICS_GLOBAL_ACCOUNT_NAME}" $ANALYTICS_AGENT_PROPERTIES
